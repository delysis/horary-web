...
The subsequent native London recovery example revealed an unexplained switch of the Moon's role and an unsupported forecast in hours. Frawley explicitly permits the Moon to signify the lost object applying to Lord 1 (pp. 147–148), so that testimony must remain available. Prompt v7 makes the role explicit, compares Lords 2 and 4 for an inanimate object, avoids assuming debility means damage, and requires stated symbolic timing reasoning. The new `london-recovery` fixture joins the five earlier hardware cases. See the pull request's validation receipt for the final six-case run; passing structural checks does not certify all of its astrology.
