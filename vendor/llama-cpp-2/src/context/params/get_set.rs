...
impl LlamaContextParams {
    ...
    /// Set the number of recurrent-state snapshots retained per sequence for rollback.
    #[must_use]
    ...
    /// Get the number of recurrent-state snapshots retained per sequence.
    #[must_use]
    pub fn n_rs_seq(&self) -> u32 {
        self.context_params.n_rs_seq
    }

    /// Configure this context as a Gemma MTP draft context.
    #[must_use]
    pub fn with_context_type_mtp(mut self) -> Self {
        self.context_params.ctx_type = llama_cpp_sys_2::LLAMA_CONTEXT_TYPE_MTP;
        self
    }

    /// Point a draft context at its target context.
    #[must_use]
    pub fn with_context_other(mut self, ctx: &LlamaContext<'_>) -> Self {
        self.context_params.ctx_other = ctx.context.as_ptr();
        self
    }
